...
// -- Car dealership --//
console.log("how many car sales were there total?", cars.length)

// Total profit for 2017
const twentySeventeenSales = cars.filter(car => car.purchase_date.includes("2017"))

console.log("how many car sales in 2017?", twentySeventeenSales.length)

// This should just be an array of numbers-- gross profits
const twentySeventeenProfitArray = twentySeventeenSales.map(salesObject => salesObject.gross_profit)
console.log(twentySeventeenProfitArray)

// Add up the gross profits
const twentySeventeenTotal = twentySeventeenProfitArray.reduce((previousValue, currentValue) => previousValue + currentValue)


console.log(twentySeventeenTotal.toFixed(2))


// In which month did they sell the most cars?
let monthWhereTheySoldTheMostCards = ""
let biggestMonthlyCountSoFar = 0
var monthlyCounts = {}

// Loop over all the car sales
// (or, depending on your interpretation of the question, loop over twentySeventeenSales)
cars.forEach(sale => {
    // Get the current month of each sale
    const currentMonth = sale.purchase_date.split("-")[1]

    // if the month of the current sale is not already a key in the object, add the key and start the counter of number of sales in this month
    if(!monthlyCounts[currentMonth]){
        monthlyCounts[currentMonth] = 1
    } else {
        // If the month of the current sale IS already in the object, just add one to the counter
        monthlyCounts[currentMonth] += 1
    }

    // You could stop right here, log the monthlyCountsObject to the console, and see which month had the highest count. BUT, if you were dealing with a larger data set, or you needed to do something with that info besides read it in the console, you'd need to figure out which month had the highest sale count with JS:

    // If the current month's sales count is bigger than the biggest sale count so far (which starts out at 0)
    if(monthlyCounts[currentMonth] > biggestMonthlyCountSoFar){
        // Then assign a new value to the biggest monthly sales count!
        biggestMonthlyCountSoFar = monthlyCounts[currentMonth]

        // And tell JS that, at this point, the month of the current sale is the highest one so far
        monthWhereTheySoldTheMostCards = currentMonth
    }
...
console.log(biggestMonthlyCountSoFar)
console.log(monthWhereTheySoldTheMostCards)
